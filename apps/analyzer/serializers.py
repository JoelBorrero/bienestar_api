...
from apps.utils.exceptions import DateValidationError, PeopleValidationError, SupervisorValidationError


def create(validated_data:dict, extra_data:dict):
    validated_data['start_date'] = validated_data['start_date'].replace(second=0, microsecond=0)
    validated_data['end_date'] = validated_data['end_date'].replace(second=0, microsecond=0)
...
class PromoterRecordSerializer(serializers.ModelSerializer):
    ...
    def create(self, validated_data):
        ...
        record, created = create(validated_data, extra_data)
        return record, created
...
class SupervisorRecordSerializer(serializers.ModelSerializer):
    ...
    def create(self, validated_data):
        extra_data = {'is_signed': True, 'supervisor': self.context['user'].account, 'was_supervised': True}
        record, created = create(validated_data, extra_data)
        return record, created

    class Meta:
        model = Record
        exclude = ('was_supervised', 'supervisor', 'is_signed', 'wake_up_calls', 'people_called')
        extra_kwargs = {
